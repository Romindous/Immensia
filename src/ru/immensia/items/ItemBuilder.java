...
public class ItemBuilder {
    ...
    public <C> ItemBuilder data(final Key key, final PersistentDataType<?, C> type, final C data) {
        ...
        pdcs.set(IStrap.key(key), type, data);
        return this;
    }
    ...
    public ItemBuilder attribute(final AttributeModifier mod) {
        final Attribute atr = Main.registries.ATTRIBS.get(mod.getKey());
        if (atr == null) {
            Main.log_warn("No attribute with key " + mod.getKey().asMinimalString());
            return this;
        }
