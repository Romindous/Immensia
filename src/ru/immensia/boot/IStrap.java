...
public class IStrap implements PluginBootstrap {
    ...
    public void bootstrap(final BootstrapContext cntx) {
        cntx.getLifecycleManager().registerEventHandler(RegistryEvents.ENCHANTMENT.entryAdd()
            // Increase the max level to 20
            .newHandler(event -> {
                final EnchantmentRegistryEntry.Builder bld = event.builder();
                bld.maxLevel(bld.maxLevel() == 1 ? 1 : bld.maxLevel() << 1);
            })
        );
    }
    ...
    public static <E extends @NotNull Keyed> Registry<E> regOf(final E val) {
        final Registry<?> reg = switch (val) {
            case final Sound ignored -> Main.registries.SOUNDS;
            case final Enchantment ignored -> Main.registries.ENCHANTS;
            case final ItemType ignored -> Main.registries.ITEMS;
            case final BlockType ignored -> Main.registries.BLOCKS;
            case final TrimMaterial ignored -> Main.registries.TRIM_TYPES;
            case final TrimPattern ignored -> Main.registries.TRIM_PATTS;
            case final EntityType ignored -> Main.registries.ENTITIES;
            case final Attribute ignored -> Main.registries.ATTRIBS;
            case final Biome ignored -> Main.registries.BIOMES;
            case final DamageType ignored -> Main.registries.DAMAGES;
            case final DataComponentType ignored -> Main.registries.COMPS;
            default -> {
                Main.log_warn("Registry of " + val.getClass().getSimpleName() + " is not defined");
                yield null;
            }
        };
    ...
    public static <T extends Keyed> Set<T> getAll(final TagKey<T> tk) {
        ...
        if (!tags.isEmpty()) {
            final List<Tag<?>> tags = IStrap.tags.get(tk.registryKey());
            if (tags != null) {
